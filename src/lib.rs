...
impl Add<Self> for PosRot {
    ...
    fn add(self, rhs: Self) -> Self {
        Self::from_floats(self.x() + rhs.x(), self.y() + rhs.y(), self.rot() + rhs.rot())
    }
}
...
impl Intermediate {
    ...
    fn lsr(&self) -> Result<Params> {
        ...
        let p_sq = (2. * self.d).mul_add(self.sa + self.sb, two.mul_add(self.c_ab, -2. + self.d_sq));
    ...
    fn rlr(&self) -> Result<Params> {
        let two: FloatType = 2.0;
        let tmp0 = (2. * self.d).mul_add(self.sa - self.sb, two.mul_add(self.c_ab, 6. - self.d_sq)) / 8.;
    ...
    fn lrl(&self) -> Result<Params> {
        let two: FloatType = 2.0;
        let tmp0 = (2. * self.d).mul_add(self.sb - self.sa, two.mul_add(self.c_ab, 6. - self.d_sq)) / 8.;
...
impl DubinsPath {
    ...
    pub fn segment(t: FloatType, qi: PosRot, type_: SegmentType) -> PosRot {
        ...
        let qt = match type_ {
            SegmentType::L => PosRot::from_floats((qi.rot() + t).sin() - st, -(qi.rot() + t).cos() + ct, t),
            SegmentType::R => PosRot::from_floats(-(qi.rot() - t).sin() + st, (qi.rot() - t).cos() - ct, -t),
    ...
    fn sample_cached(&self, t: FloatType, types: [SegmentType; 3], qi: PosRot, q1: PosRot, q2: PosRot) -> PosRot {
        let tprime = t / self.rho;
    ...
    /// ```
    #[must_use]
    pub fn sample_many_range(&self, step_distance: FloatType, range: Range<FloatType>) -> Vec<PosRot> {
    ...
    fn many_path_correctness() {
        ...
        for _ in 0..runs {
            let q0 = PosRot::from_floats(
                thread_rng.gen_range((-10000.0 as FloatType)..(10000.0 as FloatType)),
                thread_rng.gen_range((-10000.0 as FloatType)..(10000.0 as FloatType)),
                thread_rng.gen_range(((-2.0 as FloatType) * PI)..((2.0 as FloatType) * PI)),
            );
